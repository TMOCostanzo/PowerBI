...
SELECT DISTINCT newID() history_ID, JS.* --, AD.FirstName + ' ' + AD.LastName Who_Changed
INTO #CheckIt
FROM
	( 
		(	SELECT sprint_name
...
--INNER JOIN AdInfo.dbo.ActiveDirectory_Accounts AD
--	ON JS.changed_by = AD.LoginName
...
SELECT * FROM #CheckIt WHERE Added_DuringSprint = 'TRUE' and field_name LIKE 'Sprint%' --where jira_issue_key_cd = 'INFUOP-1166'-- field_name = 'Sprint' and Addition_DuringSprint = 'TRUE' 
ORDER BY jira_issue_key_cd
